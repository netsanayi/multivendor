...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <h1 class="h3 mb-0">Ürün Özelliğini Düzenle: {{ $productAttribute->name }}</h1>
    ...
    <form action="{{ route('admin.product-attributes.update', $productAttribute) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="row">
                            ...
                            <select class="form-select @error('attribute_category_id') is-invalid @enderror" 
                                ...
                                @foreach($attributeCategories as $category)
                                    <option value="{{ $category->id }}" 
                                        {{ old('attribute_category_id', $productAttribute->attribute_category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('product_category_ids') is-invalid @enderror" 
                                ...
                                @foreach($productCategories as $category)
                                    <option value="{{ $category->id }}" 
                                        {{ in_array($category->id, old('product_category_ids', $productAttribute->product_category_ids ?? [])) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div id="values-container">
                            @php
                                $values = old('values', $productAttribute->values ?? ['']);
                                ...
                                <div class="value-item mb-2">
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('values.'.$index) is-invalid @enderror" 
                                               name="values[]" placeholder="Değer girin" value="{{ $value }}" required>
                                        <button type="button" class="btn btn-danger remove-value" 
                                                style="{{ count($values) > 1 ? '' : 'display: none;' }}">
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body">
                        @if($productAttribute->image)
                            <div class="mb-3 text-center">
                                ...
                                <p class="text-muted small">Mevcut Resim</p>
                            </div>
                        @endif
                        
                        <div class="mb-3">
                            <label for="image" class="form-label">Yeni Resim Seç</label>
                        ...
                        <div id="image-preview" class="text-center" style="display: none;">
                            ...
                            <p class="text-muted small mt-2">Yeni Resim Önizlemesi</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('order') is-invalid @enderror" 
                                   id="order" name="order" value="{{ old('order', $productAttribute->order) }}" min="0" required>
                            @error('order')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" 
                                ...
                                <option value="1" {{ old('status', $productAttribute->status) == 1 ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('status', $productAttribute->status) == 0 ? 'selected' : '' }}>Pasif</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="card-body">
                        <div class="small text-muted">
                            <div class="mb-2">
                                ...
                                {{ $productAttribute->created_at->format('d.m.Y H:i') }}
                            </div>
                            <div>
                                <i class="fas fa-calendar-check"></i> 
                                <strong>Son Güncelleme:</strong> 
                                {{ $productAttribute->updated_at->format('d.m.Y H:i') }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-grid gap-2">
                    ...
                    <a href="{{ route('admin.product-attributes.show', $productAttribute) }}" class="btn btn-info">
