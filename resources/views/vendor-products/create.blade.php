...
@section('title', 'Yeni Satıcı Ürünü')
@section('page-title', 'Yeni Satıcı Ürünü')
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb mb-0">
        ...
        <li class="breadcrumb-item active" aria-current="page">Yeni Satıcı Ürünü</li>
    </ol>
</nav>
@endsection
...
<div class="container-fluid">
    <form action="{{ route('admin.vendor-products.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <input type="number" class="form-control @error('compare_price') is-invalid @enderror" 
                                       id="compare_price" name="compare_price" value="{{ old('compare_price') }}" step="0.01">
                                <span class="input-group-text">TL</span>
                            </div>
                            @error('compare_price')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control @error('max_quantity') is-invalid @enderror" 
                                   id="max_quantity" name="max_quantity" value="{{ old('max_quantity') }}" min="1">
                            @error('max_quantity')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('commission_type') is-invalid @enderror" 
                                ...
                                <option value="fixed" {{ old('commission_type') == 'fixed' ? 'selected' : '' }}>Sabit</option>
                            </select>
                ...
                <div class="admin-card mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('availability') is-invalid @enderror" 
                            ...
                            <option value="in_stock" {{ old('availability') == 'in_stock' ? 'selected' : '' }}>Stokta</option>
                            <option value="out_of_stock" {{ old('availability') == 'out_of_stock' ? 'selected' : '' }}>Stokta Yok</option>
                            <option value="pre_order" {{ old('availability') == 'pre_order' ? 'selected' : '' }}>Ön Sipariş</option>
                            <option value="discontinued" {{ old('availability') == 'discontinued' ? 'selected' : '' }}>Üretimi Durduruldu</option>
                        </select>
                        @error('availability')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" id="track_inventory" name="track_inventory" 
                               value="1" {{ old('track_inventory', true) ? 'checked' : '' }}>
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="allow_backorders" name="allow_backorders" 
                               value="1" {{ old('allow_backorders') ? 'checked' : '' }}>
