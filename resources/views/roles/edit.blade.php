        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Rol Düzenle: {{ $role->name }}</h3>
                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                           ...
                                           {{ in_array($role->name, ['Admin', 'Vendor', 'Customer']) ? 'readonly' : '' }}>
                                    @error('name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    @if(in_array($role->name, ['Admin', 'Vendor', 'Customer']))
                                        <small class="form-text text-muted">Sistem rolleri yeniden adlandırılamaz</small>
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" 
                                        ...
                                        <option value="1" {{ old('status', $role->status) == 1 ? 'selected' : '' }}>Aktif</option>
                                        <option value="0" {{ old('status', $role->status) == 0 ? 'selected' : '' }}>Pasif</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                        ...
                                        <td class="text-center">
                                            @if(in_array($permission, ['view', 'create', 'edit', 'delete']))
                                                <input type="checkbox" 
                                                       ...
                                                       {{ (isset($currentPermissions[$key]) && in_array($permission, $currentPermissions[$key])) ? 'checked' : '' }}>
                                            @endif
                                        </td>
                                        @endforeach
