...
class UpdateVendorProductRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        // Admin ve super-admin her zaman düzenleyebilir
        if (auth()->user()->hasRole(['admin', 'super-admin'])) {
            return true;
        }
        
        // Vendor sadece kendi ürünlerini düzenleyebilir
        if (auth()->user()->hasRole('vendor')) {
            return $vendorProduct && $vendorProduct->vendor_id === auth()->id();
        }
    ...
    public function rules(): array
    {
        return [
            ...
            'sku' => 'nullable|string|max:100|unique:vendor_products,sku,' . $this->route('vendor_product'),
            'barcode' => 'nullable|string|max:100|unique:vendor_products,barcode,' . $this->route('vendor_product'),
    ...
    {
        // Availability'yi quantity'ye göre güncelle
        if ($this->filled('quantity')) {
            if (!$this->filled('availability') || $this->availability === 'in_stock' || $this->availability === 'out_of_stock') {
